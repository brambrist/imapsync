...
internal/
  mailbox/           обёртка над go-imap: connect+TLS, master-login,
                     list/select/fetch/append, UID SEARCH; разбор полей, хеши
...
Список открытого технического долга — в [`TECHDEBT.md`](TECHDEBT.md). Ключевое:
таймаут на юзера не прерывает висящий IMAP-вызов; нет фолбэка при несовпадении
имени папки; оба конца синка жёстко IMAP (нет абстракции для Maildir / EWS).
