...
# sqlite_path: /var/lib/imapsync/imapsync.db   # при source: sqlite
...
4. **APPEND и внутренняя дата/флаги.** При копировании сохранять оригинальные
   INTERNALDATE и флаги письма (\Seen и пр.)? По умолчанию — да, сохранять
   оригинальную дату и флаги.
5. **Идемпотентность и рестарты.** Индекс строим каждый цикл заново из папок
   (состояние не храним локально — источник истины сами папки + X-Imapsync-Hash
   заголовок). Это упрощает демона (нет БД состояния), но требует надёжного
   дедупа. Подтвердить, что локальная БД состояния НЕ нужна.
...
Все 8 шагов реализованы (+ фича SQLite-конфига), каждый модуль с тестами,
