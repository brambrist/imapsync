...
- **Предусловие.** Сейчас демон читает списки users/folders из БД **один раз при
  старте** (`run.go` → `loadFromSQLite`); ни CLI, ни будущий API не подхватятся
  без рестарта. Сначала нужно, чтобы пул перечитывал списки из store каждый цикл
  (в режиме sqlite). Полезно само по себе.
